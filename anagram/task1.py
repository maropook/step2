...
def binary_search_word(query, dictionary):
    ...
    while left <= right:
        ...
        for i in range(common_length):
            # print(f"query:{query}, dict_word:{dict_word}")
            # print(
            #     f"left:{left} mid:{mid} right:{right} index:{i} query:{query[i]} dict:{dict_word[i]} query:{query} dict:{dict_word}"
            # )
            if ord(dict_word[i]) > ord(query[i]):
                ...
                if is_last_common_loop:  # 共通した長さまでMAX一致した場合
